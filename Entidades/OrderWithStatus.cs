...
public class OrderWithStatus
{
    ...
    public static OrderWithStatus FromOrder(Order order)
    {
        ...
        else if (DateTime.Now < dispatchTime + DeliveryDuration)
        {
            ...
            var driverPosition = LatitudLongitud.Interpolate(startPosition, order.DeliveryLocation, proportionOfDeliveryCompleted);
            mapMarkers = new List<Marker>
                                {
    ...
    private static LatitudLongitud ComputeStartPosition(Order order)
    {
        ArgumentNullException.ThrowIfNull(order.DeliveryLocation);
        ...
        return new LatitudLongitud(order.DeliveryLocation.Latitude + offset.Item1, order.DeliveryLocation.Longitude + offset.Item2);
    }

    static Marker ToMapMarker(string description, LatitudLongitud coords, bool showPopup = false)
